...
from agent_core.schemas import SwarmConfig, AgentSpec, Platform
...
class MockDriver(BaseAgentDriver):
    ...
    def _parse_response(self, raw, context):
        ...
        from agent_core.schemas import StructuredResult, ReviewFinding
...
async def main():
    ...
    parser.add_argument("--invoke-real", action="store_true", help="Use real configured model instead of MockDriver")
    parser.add_argument("--platform", type=str, default="gemini", help="Platform to use if --invoke-real is set")
    ...
    if not args.invoke_real:
        ...
        original_get_driver = agent_core.orchestrator._get_driver
    ...
    platform_map = {"gemini": Platform.GEMINI, "claude": Platform.CLAUDE_CODE, "openai": Platform.OPENAI}
    ...
    for idx, eval_task in enumerate(tasks):
        ...
        try:
            ...
            if results and results[-1].role == eval_task["expected_role"]:
                ...
                print(f"❌ FAILED: Unexpected execution path.")
